/**
 * ==============================================================================
 * BẢN QUYỀN VÀ GIẤY PHÉP ỨNG DỤNG (APPLICATION COPYRIGHT AND LICENSING)
 * ==============================================================================
 *
 * (C) 2025 [CÔNG AN XÃ MỸ AN HƯNG - TỈNH ĐỒNG THÁP - VIỆT NAM]
 *
 * MÃ NGUỒN CỐT LÕI CỦA ỨNG DỤNG NÀY (Bao gồm file script.js này và các file HTML/CSS đi kèm)
 * LÀ BẢN QUYỀN ĐỘC QUYỀN (PROPRIETARY COPYRIGHT).
 *
 * Mọi hành vi sao chép, chỉnh sửa, phân phối lại hoặc sử dụng cho mục đích thương mại
 * mà không có sự cho phép bằng văn bản đều bị nghiêm cấm.
 *
 * ==============================================================================
 * GHI CÔNG CÁC THƯ VIỆN VÀ DỮ LIỆU MÃ NGUỒN MỞ (OPEN SOURCE ATTRIBUTIONS)
 * ==============================================================================
 *
 * Ứng dụng này sử dụng các thư viện và dịch vụ mã nguồn mở, phải tuân thủ các quy định bản quyền sau:
 *
 * 1. Thư viện Bản đồ (Mapping Library):
 * - Leaflet: © 2010-2024 Vladimir Agafonkin & Leaflet Contributors (Giấy phép 2-Clause BSD).
 *
 * 2. Plugins Leaflet:
 * - Leaflet.markercluster: © 2012-2024 David Leaver & Contributors (Giấy phép MIT).
 * - Leaflet.draw: © 2012-2024 Jacob Toye, Leaflet Contributors (Giấy phép MIT).
 * - Leaflet.heat: © 2014-2024 Vladimir Agafonkin (Giấy phép BSD).
 *
 * 3. Dữ liệu Bản đồ (Map Data):
 * - Bản đồ Đường phố (OpenStreetMap): Dữ liệu © OpenStreetMap contributors (Giấy phép ODbL).
 * - Ảnh Vệ tinh (Esri World Imagery): Tiles © Esri — Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community.
 * - Bản đồ Địa hình (OpenTopoMap): Map data: © OpenStreetMap contributors, SRTM | Map style: © OpenTopoMap (Giấy phép CC-BY-SA).
 *
 * ==============================================================================
 */
let map;
...
const GOOGLE_SHEET_API_URL = 'https://script.google.com/macros/s/AKfycbxVLrhaCy0yl9YMIvXW2NYLxEkh8RmeEBte9KQ4GLByVG5J2lMP2pE-pD86T8-00l0nCg/exec';
            ...
            <div class="info-popup">
                ...
                <div class="coords">
                    ...
                    <span class="coord-actions">
                        <a href="#" onclick="event.stopPropagation(); window.shareMarker(${markerData.id}); map.closePopup();" title="Chia sẻ ghim này"><i class="fa-solid fa-share-nodes"></i></a>
...
window.deleteMarker = async function(markerId) {
    ...
    try {
        ...
        await fetch(GOOGLE_SHEET_API_URL, {
            ...
            // },
            body: JSON.stringify({ action: 'deleteMarker', markerId: markerId, token: localStorage.getItem('authToken') })
        });
    } catch (error) {
...
async function loadMarkers() {
    ...
    try {
        ...
        const typesRes = await fetch(`${GOOGLE_SHEET_API_URL}?action=getTypes&t=${new Date().getTime()}&token=${localStorage.getItem('authToken')}`);
        ...
        const versionRes = await fetch(`${GOOGLE_SHEET_API_URL}?action=getMarkersVersion&t=${new Date().getTime()}&token=${localStorage.getItem('authToken')}`);
        ...
        if (allMarkersData.length === 0 || initialOffset > 0) {
            ...
            const limit = 100;
            // Chỉ lấy total từ cache nếu đang tải dở (Kịch bản 2), nếu không thì gán bằng 0
            let total = (cachedData.status === 'incomplete' && cachedData.version === serverVersion) ? cachedData.total : 0;

            try {
                while (true) {
                    const markersRes = await fetch(`${GOOGLE_SHEET_API_URL}?action=getMarkers&offset=${offset}&limit=${limit}&t=${new Date().getTime()}&token=${localStorage.getItem('authToken')}`);
    ...
    async function saveTypes() {
        ...
        try {
            const response = await fetch(GOOGLE_SHEET_API_URL, {
                method: 'POST',
                body: JSON.stringify({ action: 'saveTypes', types: typesToSave, token: localStorage.getItem('authToken') })
            });
            const result = await response.json();
            if (result.success) {
    ...
    window.addMarker = async function(lat, lng) {
        ...
        try {
            ...
            const response = await fetch(GOOGLE_SHEET_API_URL, {
                method: 'POST',
                body: JSON.stringify({ action: 'addMarker', marker: newMarkerData, token: localStorage.getItem('authToken') })
            });
    ...
    // --- BỔ SUNG CHỨC NĂNG CHIA SẺ GHIM ---
    window.shareMarker = function(markerId) {
        // Tạo link có chứa tham số location=id
        const shareUrl = window.location.origin + window.location.pathname + '?location=' + markerId;
        
        navigator.clipboard.writeText(shareUrl).then(() => {
            alert('Đã sao chép liên kết chia sẻ:\n' + shareUrl);
        }).catch(err => {
            alert('Lỗi sao chép liên kết. Hãy thử lại.');
            console.error(err);
        });
    };
    ...
    function checkSharedLocation() {
        ...
        if (locationId) {
            const markerId = Number(locationId);
            const markerData = allMarkersData.find(m => m.id === markerId);

            if (!markerData) {
                alert('Ghim không tồn tại hoặc đã bị xóa khỏi hệ thống.');
            } else {
                // Kiểm tra quyền truy cập ghim (giống logic trong hàm masterFilter của bạn)
                let hasPermission = false;
                const userRole = currentUser.Role.toLowerCase();
                
                if (userRole === 'admin') {
                    hasPermission = true;
                } else if (userRole === 'editor' && (markerData.Owner === currentUser.Username || markerData.Owner === 'viewer')) {
                    hasPermission = true;
                } else if (userRole === 'viewer' && markerData.Owner === currentUser.Username) {
                    hasPermission = true;
                }

                if (!hasPermission) {
                    alert('Bạn không có quyền xem ghim này.');
                } else {
                    // Đợi giao diện render xong rồi mới bay tới ghim
                    setTimeout(() => {
            ...
            // Dọn dẹp URL trên trình duyệt để khi F5 không bị lặp lại hành động này
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                await fetch(GOOGLE_SHEET_API_URL, {
                    ...
                    //     'Content-Type': 'application/json',
                    // },
                    body: JSON.stringify({ action: 'updateMarker', marker: finalMarkerData, token: localStorage.getItem('authToken') })
                });
            } catch (error) {
    ...
    async function checkSession() {
        ...
        // console.log("Kiểm tra phiên đăng nhập...", storedUser, authToken);
        if (storedUser) {
    ...
    function exportVisibleMarkersToExcel() {
        ...
        markersToExport.forEach((marker, index) => {
            ...
            const inclusionDate = marker.inclusionDate ? (new Date(marker.inclusionDate)).toLocaleDateString('vi-VN') : '';
